...
from transcodon.dataset import SequenceDataset
from transcodon.pipeline import (
    Pipeline,
    DataCollator,
    DataTrimmer,
    DataPadder,
...
class CodonDataModule(pl.LightningDataModule):
    ...
    def __init__(self, args: Namespace, alphabet: Alphabet, data_dir: str,
            batch_size: int, random_seed: int = 42, test_size : float = 0.003):
        super().__init__()
    ...
    def val_dataloader(self):
        return torch.utils.data.DataLoader(self.val_data, num_workers=4,
            batch_size=self.batch_size, collate_fn=self.pipeline)
